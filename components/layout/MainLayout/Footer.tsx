      ...
      <div className="max-w-7xl mx-auto px-8 sm:px-6 py-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-6 lg:gap-10">
        ...
        <div className="space-y-4 sm:col-span-2 lg:col-span-1">
          ...
          <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
            Nền tảng kết nối tài xế và hành khách trên cùng tuyến đường, mang đến giải pháp di chuyển thông minh và tiết kiệm.
          </p>
              ...
              Object.entries(data.footer.socialLinks)
                ...
                .map(([platform, url]) => (
                  <a
                    ...
                    aria-label={platform}
                  >
                    {React.createElement(socialIcons[platform], { size: 16 })}
                  </a>
                ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-base sm:text-lg font-semibold text-foreground">Liên kết nhanh</h3>
        ...
        {/* Contact Info */}
        <div className="space-y-4">
          <h3 className="text-base sm:text-lg font-semibold text-foreground">Liên hệ</h3>
          <ul className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-muted-foreground">
            <li className="flex items-start gap-3">
              <FaPhone className="mt-0.5 flex-shrink-0" size={12} />
              <span>{data?.footer.phone || 'Đang cập nhật'}</span>
            </li>
            <li className="flex items-start gap-3">
              <FaEnvelope className="mt-0.5 flex-shrink-0" size={12} />
              <span>{data?.footer.email || 'Đang cập nhật'}</span>
            </li>
            <li className="flex items-start gap-3">
              <FaMapMarkerAlt className="mt-0.5 flex-shrink-0" size={12} />
              <span>{data?.footer.address || 'Đang cập nhật'}</span>
            </li>
          </ul>
